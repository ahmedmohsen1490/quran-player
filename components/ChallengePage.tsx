...
const ChallengePage: React.FC<ChallengePageProps> = ({
  ...
  const handleEditDuration = () => {
    const newDurationStr = window.prompt(
      'أدخل المدة الجديدة بالأيام:',
      String(challenge?.durationDays || 30)
    );
    if (newDurationStr) {
  ...
  const stats = useMemo(() => {
    ...
    const totalCompleted = Object.values(challenge.completedAyahs || {}).reduce(
      (sum, ayah) => sum + 1,
      0
    );
    ...
    const requiredPace = daysRemaining > 0 ? Math.ceil((TOTAL_QURAN_AYAHS - totalCompleted) / daysRemaining) : 0;
  ...
  const renderSetupView = () => (
    <div className="text-center">
      ...
      <p className="text-text-secondary mb-6">
        حدد المدة التي ترغب في إكمال القرآن خلالها
      </p>
      <div className="flex items-center justify-center gap-4 mb-8">
        <label htmlFor="duration" className="font-medium text-text-primary">
          أريد أن أختتم القرآن خلال
        </label>
        <input
          type="number"
          id="duration"
          ...
          className="w-24 text-center bg-background border border-border-color rounded"
        />
  ...
  const renderDashboardView = () => {
    ...
    const { totalCompleted, overallProgress, daysRemaining, requiredPace } = stats;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-center">
          ...
          <div className="col-span-2 sm:col-span-1">
            <p className="text-2xl font-bold text-primary">{requiredPace}</p>
            <p className="text-sm text-text-secondary">آية/يوم للمتابعة</p>
          </div>
        </div>

        {challenge.isPaused && (
          <div className="p-4 text-center bg-yellow-500/10 text-yellow-700">
        ...
        <div className="border-t border-border-color pt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={handlePauseToggle} className="bg-primary/20 text-primary py-2 rounded">
              {challenge.isPaused ? 'استئناف التحدي' : 'إيقاف مؤقت'}
            </button>
            <button onClick={handleEditDuration} className="bg-background text-text-primary py-2 rounded">
              تعديل المدة
            </button>
            <button onClick={handleResetChallenge} className="col-span-2 bg-red-500 text-white py-2 rounded">
              إعادة تعيين التحدي
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50">
      <div className="bg-card rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-md m-4" onClick={onClose}>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-text-primary">تحدي ختم القرآن</h2>
          <button onClick={onClose} className="text-text-secondary text-3xl leading-none">×</button>
        </div>
